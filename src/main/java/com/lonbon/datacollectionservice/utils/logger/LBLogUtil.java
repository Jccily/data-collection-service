 ...
 *
 * @Author:LonBon_XuChengYu
 * @Description:
 * @Date: 2019/6/26.
 * @Modified begin by LonBon_XuChengYu
 * ----@Date:  2019/6/26.
 * ----@Description:create.
 * @Modified end
 */
public class LBLogUtil {
    ...
    private static void redirectSystemErrInfo() throws Throwable {
        ...
        properties.load(LBLogUtil.class.getClassLoader().getResourceAsStream(LOG4J_PROPERTIES));
    ...
    private synchronized static Log getLog() {
        if(log == null) {
            log = LogFactory.getLog(LBLogUtil.class);
        }
        return log;
    }
    ...
    private static void constructMsgForDebug(StringBuffer cMsg) {
        if(getLog().isDebugEnabled()) {
            try {
                ...
                Object locationInfo = locationInfoCls.getConstructor(Throwable.class, String.class).newInstance(new Throwable(), LBLogUtil.class.getName());
                cMsg.append("[");
